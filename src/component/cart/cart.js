import "./cart.css";

function Cart({ cart, remove }) {
    return (
        <div className="cart" >
            <h4>Cart</h4>
            <hr />
            {cart.length === 0 ? <div className="cartDetails emptyCart">
                    <p>Your cart is empty</p>
                </div> : ""}
            {cart.map((el, i) => {
                console.log("map worked");
                return <Card item={el} key={"card" + el.id} itemId={el.id} remove={remove} />;
            })}
        </div>
    );
}
function Card({ item, remove, itemId }) {
    function handleRemove() {
        remove(itemId);
    }
    function handleCheckout() {
        console.log("checkout in process...");
        remove(itemId);
        // alert("checkout is in process...");
        prompt(`${item.title}\nprice: ${item.currancy + item.paymentPrice}\namount: ${item.amount} \nAdd credit card number here:`);
        alert("thank you for using our site an email receipt is sent for you")
    }
    return (
        <div className="fullCard">
            <div className="card">
                <div className="thumbnailContainer">
                    <img className="thumbnail" src={item.img} alt={item.title} />
                </div>
                <div className="cartDetails">
                    <p>{item.title}</p>
                    <p>{item.currancy + item.price + "x" + item.amount + " "}<span>{item.currancy + item.paymentPrice}</span></p>
                </div>
                <div className="delete" onClick={handleRemove}>
                    <svg width="14" height="16" xmlns="http://www.w3.org/2000/svg" xlink="http://www.w3.org/1999/xlink">
                        <defs>
                            <path d="M0 2.625V1.75C0 1.334.334 1 .75 1h3.5l.294-.584A.741.741 0 0 1 5.213 0h3.571a.75.75 0 0 1 .672.416L9.75 1h3.5c.416 0 .75.334.75.75v.875a.376.376 0 0 1-.375.375H.375A.376.376 0 0 1 0 2.625Zm13 1.75V14.5a1.5 1.5 0 0 1-1.5 1.5h-9A1.5 1.5 0 0 1 1 14.5V4.375C1 4.169 1.169 4 1.375 4h11.25c.206 0 .375.169.375.375ZM4.5 6.5c0-.275-.225-.5-.5-.5s-.5.225-.5.5v7c0 .275.225.5.5.5s.5-.225.5-.5v-7Zm3 0c0-.275-.225-.5-.5-.5s-.5.225-.5.5v7c0 .275.225.5.5.5s.5-.225.5-.5v-7Zm3 0c0-.275-.225-.5-.5-.5s-.5.225-.5.5v7c0 .275.225.5.5.5s.5-.225.5-.5v-7Z" id="a" />
                        </defs>
                        <use fill="#C3CAD9" fill-rule="nonzero" href="#a" />
                    </svg>
                </div>
            </div>
            <button className="checkout" onClick={handleCheckout}>Checkout</button>
        </div>
    );
}

export default Cart;